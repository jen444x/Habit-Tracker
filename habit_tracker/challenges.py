...
# get habits belonging to challenge
@bp.route('/<int:id>/habits')
@login_required
def challenge_habits(id):
    db = get_db()
    cur = db.cursor()

    # make sure user has this challenge
    get_challenge(id)

    # get habits from challenge
    cur.execute(
        'SELECT title, body, id ' 
        'FROM habits ' 
        'WHERE challenge_id = %s AND creator_id = %s',
        (id, g.user['id'])
    )

    challenge_habits = cur.fetchall()
    cur.close()

    print(f"challenge habits: {challenge_habits}")

    return jsonify({"challenge_habits": challenge_habits})
